...
inline const float CONVERGENCE_LIMIT        = 0.000001f;
inline const float INITIAL_RE_START         = -2.0f;
inline const float INITIAL_IM_START         = -2.0f * mainWindow::INITIAL_HEIGHT / mainWindow::INITIAL_WIDTH;
inline const float INITIAL_RE_OFFSET        = -0.78f;
inline const float INITIAL_IM_OFFSET        = -0.18f;
inline const float INITIAL_STEP             = -2.0f * INITIAL_RE_START / mainWindow::INITIAL_WIDTH;
